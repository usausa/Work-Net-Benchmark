namespace ResultMapperCacheBenchmark
{
    ...
    internal sealed class StructFieldResultMapperCache
    {
        private static readonly Node EmptyNode = new Node(typeof(EmptyKey), Array.Empty<StructFieldColumnInfo>(), default);
        ...
        public StructFieldResultMapperCache()
        {
        ...
        private static int CalculateHash(Type targetType, Span<StructFieldColumnInfo> columns)
        {
            unchecked
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsMatchColumn(StructFieldColumnInfo[] columns1, Span<StructFieldColumnInfo> columns2)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryGetValue(Type targetType, Span<StructFieldColumnInfo> columns, out object value)
        {
            var temp = nodes;
        ...
        public object AddIfNotExist(Type targetType, Span<StructFieldColumnInfo> columns, Func<Type, StructFieldColumnInfo[], object> valueFactory)
        {
            lock (sync)
            {
                ...
                var copyColumns = new StructFieldColumnInfo[columns.Length];
                columns.CopyTo(new Span<StructFieldColumnInfo>(copyColumns));
        ...
        private sealed class Node
        {
            ...
            public readonly StructFieldColumnInfo[] Columns;

            public readonly object Value;

            public Node Next;

            public Node(Type targetType, StructFieldColumnInfo[] columns, object value)
            {
