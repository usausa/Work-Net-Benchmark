namespace ResultMapperCacheBenchmark
{
    ...
    internal sealed class ClassFieldResultMapperCache
    {
        private static readonly Node EmptyNode = new Node(typeof(EmptyKey), Array.Empty<ClassFieldColumnInfo>(), default);
        ...
        public ClassFieldResultMapperCache()
        {
        ...
        private static int CalculateHash(Type targetType, Span<ClassFieldColumnInfo> columns)
        {
            unchecked
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsMatchColumn(ClassFieldColumnInfo[] columns1, Span<ClassFieldColumnInfo> columns2)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryGetValue(Type targetType, Span<ClassFieldColumnInfo> columns, out object value)
        {
            var temp = nodes;
        ...
        public object AddIfNotExist(Type targetType, Span<ClassFieldColumnInfo> columns, Func<Type, ClassFieldColumnInfo[], object> valueFactory)
        {
            lock (sync)
            {
                ...
                var copyColumns = new ClassFieldColumnInfo[columns.Length];
                columns.CopyTo(new Span<ClassFieldColumnInfo>(copyColumns));
        ...
        private sealed class Node
        {
            ...
            public readonly ClassFieldColumnInfo[] Columns;

            public readonly object Value;

            public Node Next;

            public Node(Type targetType, ClassFieldColumnInfo[] columns, object value)
            {
