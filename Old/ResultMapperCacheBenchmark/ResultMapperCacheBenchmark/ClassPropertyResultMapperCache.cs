namespace ResultMapperCacheBenchmark
{
    ...
    internal sealed class ClassPropertyResultMapperCache
    {
        private static readonly Node EmptyNode = new Node(typeof(EmptyKey), Array.Empty<ClassPropertyColumnInfo>(), default);
        ...
        public ClassPropertyResultMapperCache()
        {
        ...
        private static int CalculateHash(Type targetType, Span<ClassPropertyColumnInfo> columns)
        {
            unchecked
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsMatchColumn(ClassPropertyColumnInfo[] columns1, Span<ClassPropertyColumnInfo> columns2)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryGetValue(Type targetType, Span<ClassPropertyColumnInfo> columns, out object value)
        {
            var temp = nodes;
        ...
        public object AddIfNotExist(Type targetType, Span<ClassPropertyColumnInfo> columns, Func<Type, ClassPropertyColumnInfo[], object> valueFactory)
        {
            lock (sync)
            {
                ...
                var copyColumns = new ClassPropertyColumnInfo[columns.Length];
                columns.CopyTo(new Span<ClassPropertyColumnInfo>(copyColumns));
        ...
        private sealed class Node
        {
            ...
            public ClassPropertyColumnInfo[] Columns { get; }

            public object Value { get; }

            public Node Next { get; set; }

            public Node(Type targetType, ClassPropertyColumnInfo[] columns, object value)
            {
